...
if(count($fields)>0)
{
  ...
  function xform_geo_updates(tablem,fieldm)
  {
    ...
    link = "index.php?page=xform&subpage=manager&tripage=table_field&table_name="+table+"&geo_func=get_data&geo_field="+field+"&nocache="+currentTime.getTime();
    ...
    geocoder.geocode( { "address": address}, function(results, status) {
      if (status == google.maps.GeocoderStatus.OK) {
        if (status != google.maps.GeocoderStatus.ZERO_RESULTS) {
          ...
          data_link = "index.php?page=xform&subpage=manager&tripage=table_field&table_name="+table+"&geo_func=save_data&geo_field="+field+"&geo_lng="+lng+"&geo_lat="+lat+"&geo_id="+data_id;
          jQuery.ajax({
            url: data_link,
