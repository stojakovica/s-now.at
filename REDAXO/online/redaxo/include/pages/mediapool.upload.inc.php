    ...
    if($_FILES['file_new']['name'] != '' && $_FILES['file_new']['name'] != 'none')
    {
      if(!rex_mediapool_isAllowedMediaType($_FILES['file_new']['name'],rex_post('args','array')))
      {
        ...
        $warning .= count($whitelist)>0
                  ? '<br />'.$I18N->msg('pool_file_allowed_mediatypes').' <var>'.rtrim(implode('</var> <var>',$whitelist),', ').'</var>'
                  : '<br />'.$I18N->msg('pool_file_banned_mediatypes').' <var>'.rtrim(implode('</var> <var>',rex_mediapool_getMediaTypeBlacklist()),', ').'</var>';
      }
      else
      {
        ...
        if (rex_post('saveandexit', 'boolean') && $return['ok'] == 1)
        {
          ...
          elseif($opener_input_field != '')
          {
            if (substr($opener_input_field,0,14)=="REX_MEDIALIST_")
            {
              ...
              $js .= 'location.href = "index.php?page=mediapool&info='.urlencode($return['msg']).'&opener_input_field='.$opener_input_field.'";';
            }
            else
            {
